...
Features
  - Saves all trades to SQLite database (m42_trades.db by default)
  - Sends Telegram notifications for entries, resolutions, and errors

Usage
-----
    python 6.py                          # paper, 15m
    python 6.py --timeframe 5m           # paper, 5m
    python 6.py --balance 200             # paper $200
    python 6.py --real                    # real-money
    python 6.py --db-path m42.db          # custom db path
...
def _init_db(db_path: str) -> sqlite3.Connection:
    """Initialize the database for M42 trades."""
    con = sqlite3.connect(db_path)
    con.execute("""
        CREATE TABLE IF NOT EXISTS m42_trades (
...
def _save_entry(con: sqlite3.Connection, side: str, price: float, amount: float, 
                cl_change_pct: float, cl_price: float) -> int:
    """Save an entry to the database."""
    cursor = con.execute(
        "INSERT INTO m42_trades (side, entry_price, entry_amount, entry_ts, cl_change_pct, cl_price) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (side, price, amount, datetime.now(timezone.utc).isoformat(), cl_change_pct, cl_price)
    )
    con.commit()
    return cursor.lastrowid


def _save_resolve(con: sqlite3.Connection, trade_id: int, outcome: str, pnl: float) -> None:
    """Update a trade with resolution data."""
    con.execute(
        "UPDATE m42_trades SET outcome = ?, pnl = ?, resolve_ts = ? WHERE id = ?",
...
def main() -> None:
    ...
    trade_ids: dict[str, int] = {}  # side -> trade_id
    ...
    def m42(ctx: polyalpha.TickContext) -> None:
        ...
        # Save to database
        trade_id = _save_entry(con, side, fav_price, amount, change_pct, current_price)
        trade_ids[side] = trade_id
    ...
    def on_resolve(pos) -> None:
        ...
        if pos.side in trade_ids:
            _save_resolve(con, trade_ids[pos.side], pos.outcome, pnl)
    ...
    try:
        ...
        con.close()
        send_telegram_message(f"💥 <b>M42 Fatal Error</b>\n{exc}")
        raise
